...
ax.set_xlabel('n')
ax.set_ylabel(r'Blockaded Rabi freq. $\Omega_2/2\pi$ [MHz]')
plt.savefig('enhanced_rabi_freqs.png', bbox_inches='tight', dpi=400)

fig2, ax2 = plt.subplots(figsize=(4,3))
ax2.set_title(fr'$R$={atom_separation_distance*1e6} $\mu$m')
ax2.plot(n_array, interaction_strenghts/(2*pi*1e9))
ax2.set_xlabel('n')
ax2.set_ylabel(r'$V_{DD}/2\pi$ [GHz]')

fig3, ax3 = plt.subplots()
ax3.plot(n_array, lifetimes*1e6)
ax3.set_xlabel(r'$n$')
ax3.set_ylabel(r'Rydberg state lifetime [$\mu$s]')

fig4, ax4 = plt.subplots(figsize=(4.5, 3.5))
ax4.plot(n_array, motion_errors, '-x', label='atomic motion', alpha=0.5)
ax4.plot(n_array, blockade_errors, '-o', label='imperfect blockade', alpha=0.5)
...
ax4.set_xlabel('$n$')
ax4.set_ylabel(r'Entanglement error, $1-\mathcal{F}$')
ax4.set_yscale('log')
ax4.legend(title=r'error, $1-\mathcal{F}$')
