...
fig_width = 3.5  # inches, matches two columns
fig_height = fig_width*1.25

fig = plt.figure(figsize=(fig_width, fig_height))
# 🔹 Increased wspace from 0.35 to 0.6
gs = GridSpec(2, 2, figure=fig, wspace=0.6, hspace=0.55)

# ax2 (heatmap) is on the left, ax1 (feature plot) on the right
ax2 = fig.add_subplot(gs[0, 0])
ax1 = fig.add_subplot(gs[0, 1])
ax3 = fig.add_subplot(gs[1, 0])
ax4 = fig.add_subplot(gs[1, 1], sharey=ax3)

ax1.errorbar(x_grid_uni/MHz, surv_prob_uni[roi_idx_pick, :], sem_surv_prob_uni[roi_idx_pick, :], fmt='o', color='blue', ms=1.5)
...
cbar = fig.colorbar(im, ax=ax2, location='left', pad=0.25)
cbar.set_label(r' AC Stark Shift [MHz]')

# --- CHANGE 1: Removed x index and y index labeling ---
...
ax3.errorbar(x_grid_trapfreq_ax/kHz, glob_surv_trapfreq_ax, yerr=glob_surv_sem_trapfreq_ax, fmt='o', ms=1.5,color='blue')
ax3.plot(x_axis_fit_trapfreq_ax/kHz, FittingFunctions.lorentzian(x_axis_fit_trapfreq_ax, *popt_trapfreq_ax), color='darkblue')

ax4.errorbar(x_grid_trapfreq_rad/kHz, glob_surv_trapfreq_rad, yerr=glob_surv_sem_trapfreq_rad, fmt='o', ms=1.5, color='orange')
ax4.plot(x_axis_fit_trapfreq_rad/kHz, FittingFunctions.lorentzian(x_axis_fit_trapfreq_rad, *popt_trapfreq_rad), color='darkorange')
...
fig.text(0.5, 0.02, 'Modulation frequency (kHz)', ha='center')
ax3.set_ylabel('Avg. survival probability')
