...
def main(folder, file_name, color, show_gaussian_fit):
    # load image
    ...
    pixels_x = pixels_y = np.linspace(-crop_r/2, (crop_r - 1)/2, crop_r)

    # guess for fittin data
    fitting_guess = [0.2, 0.8, 0, 10]  # offset, amplitude, middle, width
    ...
    sigma_y_px = popt_rows[3]
    sigma_x = CameraImage.pixels_to_m(sigma_x_px, cam_mag, pixel_size, bin_size)
    sigma_y = CameraImage.pixels_to_m(sigma_y_px, cam_mag, pixel_size, bin_size)
    print(str(np.round(sigma_x*1e6)) + ' um')
    print(str(np.round(sigma_y*1e6)) + ' um')
    ...
    # generate x,y axis true to scale for plot
    axis_x = CameraImage.pixels_to_m(pixels_x, cam_mag, pixel_size, bin_size)
    axis_y = CameraImage.pixels_to_m(pixels_y, cam_mag, pixel_size, bin_size)
    ...
    if show_gaussian_fit:
        ...
        ax2.scatter(-(hist_rows), axis_y*1e3, s=7)
        ax2.plot(-(FittingFunctions.gaussian_function(pixels_y, *popt_rows)),
            axis_y*1e3, color='r', linewidth=1)

        # plot sum over columns as well as gaussian fit
        # plot in mm (multiply 1e3)
        ax3.scatter(axis_x*1e3, hist_cols, s=7)
        ax3.plot(-np.flip(axis_x*1e3), 
...
if __name__ == '__main__':
    ...
    # importing data
    folder = redmot_sf_folder
    file_name = redmot_sf_file_name

    main(folder, file_name, color_plot, show_gaussian_fit=False)
