...
class NumberAtoms:
    ...
    def atom_number_from_image(fit_parameters_x, fit_parameters_y,
                               max_counts, gain, exp_time, constant_counts, loss_constant,
                               linewidth,
                               lens_dist, lens_rad):
        ...
        counts_roi = 2 * np.pi * amplitude_weighted * sigma_horiz * sigma_vert
        ...
        collection_part = 4 * np.pi * lens_dist**2 / lens_rad**2
    ...
    def atomic_density_from_atom_number(atom_number, sigma_horiz, sigma_vert):
        ...
        atom_density_si_units = atom_number / (2 * np.pi)**(3/2) / (sigma_x_m**2 * sigma_y_m)
